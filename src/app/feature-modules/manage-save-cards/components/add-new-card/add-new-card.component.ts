...
export class AddNewCardComponent implements OnInit {
  ...
  validateForm() {
    ...
    const isValidCardNumber = this.srv?.validCardNumber(this.addNewCardForm.value.cardNumber);

    if (!this.addNewCardForm.value.cardNumber || (this.addNewCardForm.value.cardNumber.length < 19 || !isValidCardNumber)) {
    ...
    else if (this.addNewCardForm.value.expireMonth < month) {
      this.error['expireMonthError'] = ERROR_MSG.MONTH_EXPIRED;
      return false;
    }
    else if (this.addNewCardForm.value.expireYear === year && (this.addNewCardForm.value.expireMonth < month)) {
      this.error['expireMonthError'] = ERROR_MSG.MONTH_EXPIRED;
      return false;
    }
    ...
    else if (this.addNewCardForm.value.expireYear < year) {
      this.error['expireYearError'] = ERROR_MSG.YEAR_EXPIRED;
      return false;
    }
